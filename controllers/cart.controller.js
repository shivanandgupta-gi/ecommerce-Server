...
export async function addToCartItem(request, response) {
    try {
        ...
        const { productId, productTitle, image, rating, price, quantity, subTotal, countInStock, } = request.body;  //in {} means  extract productId only
...
export async function updateCartItemQut(request, response) {
    try {
        ...
        const { _id, qty ,subTotal} = request.body; //id of cart and how much quantity from body
...
//for empty cart when once order placed
